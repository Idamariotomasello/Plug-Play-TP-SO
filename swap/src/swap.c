...
bool swap_escribir_bloque(int32_t numero_bloque, void *datos)
{
    ...
    {
        log_error(logger, "Bloque %d fuera de rango (swap_size=%d, block_size=%d)",
 ...
 *     KM envía: numero_bloque (int32_t) | datos (block_size bytes)
 *     SWAP responde: OP_OK | OP_ERROR
 *
 *   OP_SWAP_LEER_BLOQUE (501):
 *     KM envía: numero_bloque (int32_t)
 *     SWAP responde: OP_OK | datos (block_size bytes)
 *                 o: OP_ERROR
 * ========================================================= */
...
void swap_atender_km(void)
{
    ...
    while (recibir_int32(fd_km, &cod_op))
    {
        switch (cod_op)
        {
            case OP_SWAP_ESCRIBIR_BLOQUE: {
                /* Recibir número de bloque */
                ...
                {
                    log_error(logger, "Error recibiendo datos del bloque %d", numero_bloque);
                    free(datos);
                    enviar_int32(fd_km, OP_ERROR);
                    break;
                }
...
/* =========================================================
 * swap_crear_archivo
 * Crea (o trunca) el archivo de swap al tamaño configurado.
 * El contenido se asume libre — no se inicializa.
 * ========================================================= */

bool swap_crear_archivo(const char *path, int32_t tamanio)
{
    /* Abrir en modo r+b si existe, wb si no */
    ...
    /* Expandir al tamaño configurado (sin inicializar contenido) */
    ...
    /* Reabrir en r+b para lectura/escritura sin truncar */
    fclose(g_swap_file);
    ...
    int bloques_totales = tamanio / g_block_size;
    log_info(logger, "Archivo de SWAP listo: '%s' — %d bytes, %d bloques de %d bytes",
             path, tamanio, bloques_totales, g_block_size);
    return true;
}
 
/* =========================================================
 * main
 * Uso: ./swap swap.config
 * ========================================================= */
...
int main(int argc, char *argv[])
{
    ...
    if (g_block_size <= 0 || g_swap_size <= 0 || g_swap_size % g_block_size != 0)
    {
        log_error(logger,
                  "Configuracion invalida: SWAP_FILE_SIZE=%d debe ser multiplo de BLOCK_SIZE=%d",
    ...
    log_info(logger, "SWAP configurado: archivo='%s', tamanio=%d bytes, bloque=%d bytes",
             swap_path, g_swap_size, g_block_size);
    ...
    /*
     * Enviar metadatos al KM — dos int32_t post-handshake:
     *   1. block_size
     *   2. swap_size
     * El KM los recibe en km_atender_swap() con dos recibir_int32.
     */
    enviar_int32(fd_km, g_block_size);
    enviar_int32(fd_km, g_swap_size);
 
    log_info(logger, "## Conectado a Kernel Memory — bloque: %d bytes, total: %d bytes",
             g_block_size, g_swap_size);
 
    /*
     * Atender pedidos del KM en el hilo principal.
     * swap_atender_km() es bloqueante y solo retorna
     * si el KM se desconecta.
     */
    swap_atender_km();
